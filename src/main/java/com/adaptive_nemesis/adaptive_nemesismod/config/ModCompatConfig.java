...
public class ModCompatConfig {
    ...
    */
    public final ModConfigSpec.BooleanValue MOD_COMPAT_L2HOSTILITY_ENABLED;

    /**
    * 是否启用史诗战斗 (Epic Fight) 兼容模式
    * 启用后，会应用专用权重计算逻辑，防止怪物被击飞过远
    */
    public final ModConfigSpec.BooleanValue MOD_COMPAT_EPIC_FIGHT_ENABLED;

    /**
    * 是否启用铁魔法 (Irons Spells) 兼容模式
    * 启用后，会抑制刷屏式 DEBUG 日志输出
    */
    public final ModConfigSpec.BooleanValue MOD_COMPAT_IRONS_SPELLS_ENABLED;

    /**
    * 是否启用神化 (Apotheosis) 兼容模式
    * 启用后会在玩家强度评估中考虑神话词条加成
    */
    public final ModConfigSpec.BooleanValue MOD_COMPAT_APOTHEOSIS_ENABLED;

    public ModCompatConfig(ModConfigSpec.Builder builder) {
